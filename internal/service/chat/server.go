...
import (
	...
	"WebsocketChat/pkg/enum/message/message_status_enum"
	"WebsocketChat/pkg/enum/message/message_type_enum"
		...
		case client := <-s.Login:
			{
				...
				s.mutex.Unlock()
				zlog.Debug(fmt.Sprintf("欢迎来到websocket聊天服务器，亲爱的用户%s\n", client.Uuid))
				err := client.Conn.WriteMessage(websocket.TextMessage, []byte("欢迎来到websocket聊天服务器"))
				if err != nil {
