...
import (
	...
	"errors"
	"fmt"
	"WebsocketChat/internal/dao"
	"WebsocketChat/internal/dto/request"
	"WebsocketChat/internal/dto/respond"
	"WebsocketChat/internal/model"
	myredis "WebsocketChat/internal/service/redis"
	"WebsocketChat/pkg/constants"
	"WebsocketChat/pkg/enum/contact/contact_status_enum"
	"WebsocketChat/pkg/enum/contact/contact_type_enum"
	"WebsocketChat/pkg/enum/contact_apply/contact_apply_status_enum"
	"WebsocketChat/pkg/enum/group_info/group_status_enum"
	"WebsocketChat/pkg/enum/user_info/user_status_enum"
	"WebsocketChat/pkg/util/random"
...
func (u *userContactService) GetUserList(ownerId string) (string, []respond.MyUserListRespond, int) {
	...
	if err == nil {
		...
		if err := json.Unmarshal([]byte(rspString), &userListRsp); err != nil {
			zlog.Error("解析缓存数据失败: " + err.Error())
			// 解析失败，回退到数据库
		} else {
		...
		// Redis连接错误，记录日志但不中断
		zlog.Error("Redis操作失败: " + err.Error())
	}
	...
	for _, contact := range contactList {
		// 联系人中是用户的
		if contact.ContactType == contact_type_enum.USER { // 使用正确的枚举值
			// 获取用户信息
			var user model.UserInfo
			if res := dao.GormDB.First(&user, "uuid = ?", contact.ContactId); res.Error != nil {
				// 记录错误但不中断
				zlog.Error("获取用户信息失败: " + res.Error.Error())
				continue
			}
	...
	if len(userListRsp) > 0 {
		...
		if err != nil {
			...
			if err := myredis.SetKeyEx("contact_user_list_"+ownerId, string(rspString), time.Minute*constants.REDIS_TIMEOUT); err != nil {
				// 缓存失败不影响业务逻辑，仅记录日志
				zlog.Error("缓存联系人列表失败: " + err.Error())
			}
		}
	}
