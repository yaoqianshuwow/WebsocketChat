...
export default {
  ...
  setup() {
	...
	console.log("[WebsocketChat] 后端URL:", store.state.backendUrl, "WebSocket URL:", store.state.wsUrl);
        const response = await axios.post(
          store.state.backendUrl + "/login",
          data.loginData
        );
        console.log("[WebsocketChat] 登录响应:", response);
        if (response.data.code == 200) {
          ...
          try {
            ...
            console.error("[WebsocketChat] 登录处理异常:", error);
          }
        } else {
          ElMessage.error(response.data.message);
        }
      } catch (error) {
        console.error("[WebsocketChat] 登录请求异常:", error);
