  ...
  setup() {
    ...
    const handleRegister = async () => {
      try {
        ...
        console.log("[WebsocketChat] 注册响应:", response);
        if (response.data.code == 200) {
          ElMessage.success(response.data.message);
          console.log("[WebsocketChat] 注册成功:", response.data.message);
          // 查看avatar前缀有没有http
          ...
          console.error("[WebsocketChat] 注册失败:", response.data.message);
        }
      } catch (error) {
        console.error("[WebsocketChat] 注册请求异常:", error);
    ...
    const sendSmsCode = async () => {
      ...
      console.log("[WebsocketChat] 发送验证码响应:", rsp);
