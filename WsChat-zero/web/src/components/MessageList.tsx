...
export default function MessageList() {
  ...
  if (!currentSession) {
    return <div style={styles.empty}>选择一个会话开始聊天</div>;
  }
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>{currentSession.sessionName}</div>
      <div style={styles.list}>
        {messages.map((msg: MessageVo) => {
          const isMe = msg.senderId === user?.userId || msg.senderId === user?.user_id;
          return (
            <div key={msg.msgId} style={{ ...styles.msgRow, ...(isMe ? styles.msgRowMe : {}) }}>
              <div style={styles.bubble}>
                <div style={styles.sender}>{isMe ? '我' : msg.senderId}</div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  header: { padding: '12px 16px', fontWeight: 600, fontSize: 14, borderBottom: '1px solid #e8e8e8', background: '#fff', color: '#333' },
  list: { flex: 1, overflow: 'auto', padding: '16px', background: '#fafafa' },
