// chat_pair_test.go — 单聊 + 群聊全流程自动化测试
// 测试 u1 ↔ u2 单聊，和 u1+u2 群聊的消息收发、WebSocket 实时推送、消息持久化
//
// 使用方式：go run test/chat_pair_test.go
//
// 前置条件：
...
func createGroupAndAddMember() {
	...
	if !u2InGroup {
		// 搜索群聊让 u2 加群
		var searchGroupResp GroupSearchResp
		apiPost("/api/v1/group/searchGroupList", map[string]any{"keyword": "auto-test-group"}, u2.Token, &searchGroupResp)
		var joinResp struct {
			Code    int32  `json:"code"`
			Message string `json:"message"`
		}
		apiPost("/api/v1/group/joinGroup", map[string]any{"groupId": groupID}, u2.Token, &joinResp)
		must(joinResp.Code == 0, "u2 加群失败: %s", joinResp.Message)
	}
